...
#include "modules/perception/3rd_lib_ubuntu16/xlog/include/xlog.h"
...
bool Cipv::distance_from_point_to_line_segment(
  ...
  *distance = sqrt(dx * dx + dy * dy);
  if (_debug_level >= 2) {
    XLOG(INFO) << "distance_from_point (" << point[0] << ", " << point[1]
...
bool Cipv::get_egolane(const lane_post_process::LaneObjectsPtr lane_objects,
                       EgoLane *egolane_image, EgoLane *egolane_ground,
                       bool *b_left_valid, bool *b_right_valid) {
  for (size_t i = 0; i < lane_objects->size(); ++i) {
    if ((*lane_objects)[i].spatial == lane_post_process::L_0) {
      if (_debug_level >= 2) {
        XLOG(INFO) << "[get_egolane]LEFT(*lane_objects)[i].image_pos.size(): "
    ...
    } else if ((*lane_objects)[i].spatial == lane_post_process::R_0) {
      if (_debug_level >= 2) {
        XLOG(INFO) << "[get_egolane]RIGHT(*lane_objects)[i].image_pos.size(): "
                   << (*lane_objects)[i].image_pos.size();
      }
      if ((*lane_objects)[i].image_pos.size() <
          MIN_LANE_LINE_LENGTH_FOR_CIPV_DETERMINATION) {
...
float Cipv::vehicle_dynamics(const int tick, const float yaw_rate,
...
bool Cipv::make_virtual_ego_lane_from_yaw_rate(const float yaw_rate,
  ...
  if (_b_image_based_cipv == false) {
    for (uint32_t i = 0; i < 120; i += 5) {
      vehicle_dynamics(i, velocity, yaw_rate, _time_unit, x, y);
...
bool Cipv::elongate_egolane(
    const lane_post_process::LaneObjectsPtr lane_objects,
  ...
  if (b_left_valid && b_right_valid) {
    ...
    make_virtual_ego_lane_from_yaw_rate(yaw_rate, velocity, offset_distance,
                                        egolane_ground->left_line,
                                        egolane_ground->right_line);
    XLOG(INFO) << "Made both lane_objects";
  }
...
bool Cipv::find_closest_edge_of_object_ground(
    ...
    LineSegment2Df *closted_object_edge) {
  if (_debug_level >= 2) {
    XLOG(INFO) << "object->track_id: " << object->track_id;
    // XLOG(INFO) << "object->length: " << object->length;
    // XLOG(INFO) << "object->width: " << object->width;
    // XLOG(INFO) << "object->height: " << object->height;
  }
  float size_x = object->length;
  float size_y = object->width;
  float size_z = object->height;
  ...
  if (_debug_level >= 3) {
    XLOG(INFO) << "object->center[0]: " << object->center[0];
    XLOG(INFO) << "object->center[1]: " << object->center[1];
    XLOG(INFO) << "object->center[2]: " << object->center[2];
    XLOG(INFO) << "object->direction[0]: " << object->direction[0];
    XLOG(INFO) << "object->direction[1]: " << object->direction[1];
    XLOG(INFO) << "object->direction[2]: " << object->direction[2];
  }
  float center_x = object->center[0];
  float center_y = object->center[1];
  float direction_x = object->direction[0];
  ...
  if (_debug_level >= 2) {
    XLOG(INFO) << "P0(" << p[0][0] << ", " << p[0][1] << ")";
    XLOG(INFO) << "P1(" << p[1][0] << ", " << p[1][1] << ")";
    XLOG(INFO) << "P2(" << p[2][0] << ", " << p[2][1] << ")";
    XLOG(INFO) << "P3(" << p[3][0] << ", " << p[3][1] << ")";
  }
  ...
  if (_debug_level >= 2) {
    XLOG(INFO) << "start(" << closted_object_edge->start_point[0] << ", "
               << closted_object_edge->start_point[1] << ")->";
    XLOG(INFO) << "end(" << closted_object_edge->end_point[0] << ", "
               << closted_object_edge->end_point[1] << ")";
  }
  return true;
}
...
bool Cipv::are_distances_sane(const float distance_start_point_to_right_lane,
  ...
  if (distance_start_point_to_right_lane > MAX_DIST_OBJECT_TO_LANE_METER) {
    if (_debug_level >= 1) {
      XLOG(INFO) << "distance from start to right lane(" << distance
                 << " m) is too long";
    }
    return false;
  }
  if (distance_start_point_to_left_lane > MAX_DIST_OBJECT_TO_LANE_METER) {
    if (_debug_level >= 1) {
      XLOG(INFO) << "distance from start to left lane(" << distance
                 << " m) is too long";
    }
    return false;
  }
  if (distance_end_point_to_right_lane > MAX_DIST_OBJECT_TO_LANE_METER) {
    if (_debug_level >= 1) {
      XLOG(INFO) << "distance from end to right lane(" << distance
                 << " m) is too long";
    }
    return false;
  }
  if (distance_end_point_to_left_lane > MAX_DIST_OBJECT_TO_LANE_METER) {
    if (_debug_level >= 1) {
      XLOG(INFO) << "distance from end to left lane(" << distance
                 << " m) is too long";
    }
    return false;
  }
  ...
  if (distance > MAX_VEHICLE_WIDTH_METER) {
    if (_debug_level >= 1) {
      XLOG(INFO) << "width of vehicle (" << distance << " m) is too long";
    }
    return false;
  }
...
bool Cipv::is_point_left_of_line(const Point2Df &point,
  ...
  if (cross_product > 0.0f) {
    if (_debug_level >= 2) {
      XLOG(INFO) << "point (" << point[0] << ", " << point[1]
...
bool Cipv::is_object_in_the_lane_ground(const ObjectPtr &object,
  ...
  if (!b_valid_object) {
    if (_debug_level >= 1) {
      XLOG(DEBUG) << "The closest edge of an object is not available";
    }
    return false;
  }

  if (_debug_level >= 3) {
    XLOG(INFO) << "egolane_ground.left_line.line_point.size(): "
               << egolane_ground.left_line.line_point.size();
  }
  if (egolane_ground.left_line.line_point.size() <= 1) {
    if (_debug_level >= 1) {
      XLOG(DEBUG) << "No left lane";
    }
    return false;
  }
  ...
  if (closest_index >= 0) {
    ...
    if (_debug_level >= 3) {
      XLOG(INFO) << "[Left] closest_index: " << closest_index
  ...
  if (closest_index >= 0) {
    if (_debug_level >= 3) {
      XLOG(INFO) << "[right] closest_index: " << closest_index
                 << ", shortest_distance: " << shortest_distance;
    }
    // Check if the end point is on the right of the line segment
    if (is_point_left_of_line(
...
bool Cipv::determine_cipv(std::shared_ptr<SensorObjects> sensor_objects,
                          CipvOptions *options) {
  if (_debug_level >= 3) {
    XLOG(INFO) << "Cipv Got SensorObjects size "
               << sensor_objects->objects.size();
    XLOG(INFO) << "Cipv Got lane object size "
  ...
  // XLOG(INFO)<<"static_cast<int32_t>(sensor_objects->objects.size(): "
  ...
  for (int32_t i = 0; i < static_cast<int32_t>(sensor_objects->objects.size());
    ...
    if (is_object_in_the_lane(sensor_objects->objects[i], egolane_image,
      ...
      if (cipv_index >= 0) {  // if there is existing CIPV
        ...
        // XLOG(INFO) << "sensor_objects->objects[i]->center[0]: "
        //            << sensor_objects->objects[i]->center[0];
        // XLOG(INFO) << "sensor_objects->objects[cipv_index]->center[0]: "
      ...
      if (_debug_level >= 2) {
        XLOG(INFO) << "current cipv_index: " << cipv_index;
      }
    }
  }
  // XLOG(INFO) << "old_cipv_index: " << old_cipv_index;
  if (old_cipv_index != cipv_index && cipv_index >= 0) {
    // XLOG(INFO) << "sensor_objects->objects[cipv_index]->b_cipv: "
    //            << sensor_objects->objects[cipv_index]->b_cipv;
    // XLOG(INFO) << "sensor_objects->cipv_index: "
    //            << sensor_objects->cipv_index;
    // XLOG(INFO) << "sensor_objects->cipv_track_id: "
    //            << sensor_objects->cipv_track_id;
    if (old_cipv_index >= 0) {
      // XLOG(INFO) << "sensor_objects->objects[old_cipv_index]->b_cipv: "
    ...
    if (_debug_level >= 1) {
      XLOG(INFO) << "final cipv_index: " << cipv_index;
      XLOG(INFO) << "final cipv_track_id: " << cipv_track_id;
      // XLOG(INFO) << "CIPV Index is changed from " << old_cipv_index << "th
      // object to "
      //            << cipv_index << "th object.";
      // XLOG(INFO) << "CIPV Track_ID is changed from " << old_cipv_track_id <<
      // " to "
      //            << cipv_track_id << ".";
    }
  } else {
